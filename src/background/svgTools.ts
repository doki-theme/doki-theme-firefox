...
const svgUrlToPng = (svgUrl: string, options: FaviconOptions): Promise<any> => {
...
const buildSVG = (dokiTheme: DokiTheme, { width, height }: FaviconOptions) =>
...
export const svgToPng = (dokiTheme: DokiTheme, options: FaviconOptions): Promise<any> => {
  const url = getSvgUrl(buildSVG(dokiTheme, options));
  return svgUrlToPng(url, options).then(imageBlob => {
